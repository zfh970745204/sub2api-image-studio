...
from app.services.asset_files import AssetInputError, PreparedAsset, make_thumbnail
...
class AssetService:
    ...
    async def store(
        ...
        *,
        ...
        if edit_source is not None and (
            edit_source.extension != "png"
            or (edit_source.width, edit_source.height) != (prepared.width, prepared.height)
        ):
            raise AssetInputError("选区修边原图必须为与结果同尺寸的 PNG")
        ...
        try:
            ...
            if edit_source is not None:
                await storage.put_object(
                    self.edit_source_key(object_key),
                    edit_source.data,
                    content_type="image/png",
                    metadata={"asset-id": str(asset_id), "purpose": "cutout-source"},
                )
            await storage.put_object(
                object_key,
                prepared.data,
        ...
        async with database.session_factory() as session:
            ...
            if (
                ...
                or (
                    ...
                    and str(asset.id) not in (job.output_asset_ids or [])
                )
            ):
                return False
            ...
            for asset in rows:
                ...
                if object_valid and (
                    ...
                    or (
                        ...
                        and (
                            source_job.output_asset_id == asset.id
                            or str(asset.id) in (source_job.output_asset_ids or [])
                        )
                    )
                ):
                    asset.status = "ready"
