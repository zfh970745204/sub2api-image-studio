...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE image_jobs SET output_asset_ids = jsonb_build_array(output_asset_id::text) WHERE output_asset_id IS NOT NULL"
    )
    op.execute("""
    ...
    op.execute(
        ...
        """).bindparams(
            parameter_rules=(
                '{"rules":[{"parameter":"quality","type":"choice",'
                '"points":{"low":0,"medium":0,"high":10,"auto":10}}]}'
            )
        )
    )
