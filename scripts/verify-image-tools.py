...
async def verify_cutout(settings, source, directory):
    runner = BackgroundRemovalRunner(settings)
    ...
    async def run():
        ...
        async with asyncio.timeout(180):
            raw = await runner.remove(source)
    ...
    # Match the production runner, including two jobs sharing its single slot.
    return {
        "cutout_startup_seconds": startup_seconds,
        "cutout_seconds": await asyncio.gather(run(), run()),
        "cutout_threads": settings.background_model_threads,
    }
...
def main():
    ...
    metrics = {
        ...
        "product_color": product_background(source_raw),
    }
    if not args.skip_cutout:
        metrics.update(asyncio.run(verify_cutout(settings, source_raw, directory)))
    ...
    draw.text(
        (20, 10),
        "Product-color extraction - local fixtures, not a new AI generation",
        fill="black",
    )
    reports = []
    for row, background in enumerate(("#000000", "#FFFFFF")):
        ...
        transparent, _ = finish_print(
            encoded(flat), mode="transparent", color=background
        )
        with Image.open(BytesIO(transparent)) as result:
            recomposed = Image.alpha_composite(canvas, result).convert("RGB")
            delta = np.abs(
                np.asarray(recomposed).astype(int) - np.asarray(flat).astype(int)
            )
            reports.append(
                {
                    "background": background,
                    "mean_channel_difference_on_product": float(delta.mean()),
            ...
            for column, (preview, label) in enumerate(
                zip(
                    previews,
                    (
                        "Opaque product background",
                        "Transparent PNG preview",
                        "On the same product color",
                    ),
                    strict=True,
                )
            ):
