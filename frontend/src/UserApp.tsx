        ...
        <aside className="user-studio-controls">
          ...
          {meta.source && (
            <section className="studio-source-picker" aria-label="来源素材">
              ...
              <button className="studio-source-choice" aria-label="从素材库选择图片" onClick={() => setAssetPicker("source")} type="button">{source ? <><ImageThumbnail id={source.id} /><span><strong>{source.original_filename || operationName(source.operation_code)}</strong><small>{source.width} × {source.height} · 点击更换</small></span></> : <><span className="studio-source-empty"><Images size={23} /></span><span><strong>从素材库选择图片</strong><small>浏览缩略图，选择要处理的素材</small></span></>}<ChevronDown size={16} /></button>
            </section>
          )}
          ...
          {operationCode === "ai.extract_print" && <div className="studio-settings-pair">
            ...
            <Segmented label="生成质量" value={form.quality} options={[["medium", "标准"], ["high", "精细"]]} onChange={(value) => setForm({ ...form, quality: value })} />
          </div>}
          {operationCode === "ai.extract_print" && <label className="user-field studio-setting-row"><span className="studio-field-label">输出尺寸<InfoHint label="输出尺寸说明">按比例适配画布，完整保留图案；放大尺寸不等于增加原始细节。</InfoHint></span><select aria-label="输出尺寸" value={form.printOutputSize} onChange={(event) => setForm({ ...form, printOutputSize: event.target.value })}><option value="2048x2048">方形 · 2048 × 2048</option><option value="2048x3072">竖版 · 2048 × 3072</option><option value="3072x2048">横版 · 3072 × 2048</option><option value="3072x3072">大方形 · 3072 × 3072</option></select></label>}
          {isGeneration && <div className="studio-settings-pair"><label className="user-field"><span>画布尺寸</span><select onChange={(event) => setForm({ ...form, size: event.target.value })} value={form.size}><option value="1024x1024">1024 × 1024</option><option value="1024x1536">1024 × 1536</option><option value="1536x1024">1536 × 1024</option><option value="auto">自动</option></select></label><Segmented label="生成质量" value={form.quality} options={[["medium", "标准"], ["high", "精细"]]} onChange={(value) => setForm({ ...form, quality: value })} /></div>}
          {operationCode.startsWith("ai.") && !isGeneration && operationCode !== "ai.extract_print" && <div className="studio-setting-row"><Segmented label="生成质量" value={form.quality} options={[["medium", "标准"], ["high", "精细"]]} onChange={(value) => setForm({ ...form, quality: value })} /></div>}
          ...
          <div className="user-quote-summary"><span>预计积分<InfoHint label="积分扣费说明">{operationCode === "ai.ecommerce" && `共 ${form.imageCount} 张。`}确认报价后扣费，失败自动退还积分。任务提交后可继续创作，超出并发上限会自动排队。</InfoHint></span><strong>{estimatedPoints(selectedOperation, parameters()) ?? "--"}<small>积分</small></strong></div>
...
function PreviewBackgroundControls({
  ...
  return (
    <section className="user-preview-controls">
      <header><span>效果图底色</span><InfoHint label="预览背景说明">只影响右侧效果图的显示，不改变原图、生成参数或导出文件。不透明图片需先抠图才能看到底色。</InfoHint></header>
