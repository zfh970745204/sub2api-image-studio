...
export function AssetPickerDialog({ title = "选择素材", selectedId, excludedIds = [], onSelect, onClose }: AssetPickerDialogProps) {
  ...
  >
    <header className="asset-picker-header">
      <div><h2 id={titleId}>{title}</h2><p id={descriptionId}>从素材库选择一张图片，确认后使用。</p></div>
          ...
          : !pager.items?.length ? <div className="asset-picker-state" role="status"><Images size={32} aria-hidden="true" /><p>暂无{kind === "original" ? "原图" : kind === "result" ? "处理结果" : "可选素材"}</p><span>可以切换类型，或上传图片后再来选择。</span></div>
            : <ul className="asset-picker-grid" aria-label="素材列表">
              {pager.items.map((asset) => {
                ...
                const active = selection?.id === asset.id;
                const name = filename(asset);
                return <li key={asset.id}>
                  <button className="asset-picker-card" type="button" aria-label={`选择 ${name}${unavailable ? "（已添加）" : ""}`} aria-pressed={active} disabled={unavailable} onClick={() => setSelected(asset)}>
    ...
    <footer className="asset-picker-footer">
      <div className="asset-picker-selection" role="status">
        {selection ? <><span className="asset-picker-selection-image"><ImageThumbnail id={selection.id} alt="已选素材缩略图" /></span><span><strong title={filename(selection)}>已选择：{filename(selection)}</strong><small>{dimensions(selection)}</small></span></> : <span>请选择一张素材</span>}
      </div>
      <div className="asset-picker-actions"><button type="button" onClick={onClose}>取消</button><button className="asset-picker-confirm" type="button" disabled={!selection} onClick={() => { if (selection) onSelect(selection); }}>确认选择</button></div>
    </footer>
