...
const frame = (selected: number | null = null, changed = false): RasterReply => ({ type:"frame", preview:bitmap(), overlay:bitmap(), original:bitmap(), state: { canUndo:changed || selected !== null, canRedo:false, changed, selected, visible:1000 } });
...
describe("basic editor workflow", () => {
  ...
  it("loads the current version once, maps shape coordinates, and fills only the selection", async () => {
    ...
    expect(FakeWorker.latest.postMessage).toHaveBeenLastCalledWith({ type:"action", action:{ type:"shape", shape:"rectangle", points:[{x:20,y:20},{x:100,y:100}], mode:"add" } });
  ...
  it("saves once through the manual endpoint, retries failure, and protects unsaved edits on close", async () => {
    ...
    FakeWorker.latest.emit({ type:"export", blob:new Blob(["edited"], { type:"image/png" }) });
