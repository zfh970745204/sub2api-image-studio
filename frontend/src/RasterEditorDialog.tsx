...
type Tool = "rectangle" | "ellipse" | "lasso" | "wand" | "brush" | "eraser" | "eyedropper" | "hand";
const TOOLS = [
...
export function RasterEditorDialog({ asset, previewUrl, onClose, onSaved }: { asset: Asset; previewUrl?: string | null; onClose: () => void; onSaved: (asset: Asset) => void }) {
    ...
    async function load() {
      try {
        ...
        instance.onmessage = async ({ data }: MessageEvent<RasterReply>) => {
          ...
          if (data.type === "export") {
            try { const result = await api.saveRasterEdit(asset.id, data.blob); if (alive) saved.current(result.asset); }
        ...
        const blob = await selectionPixels(`/api/v1/assets/${asset.id}/selection/result`, controller.signal);
        if (alive) instance.postMessage({ type: "init", blob, width, height } satisfies RasterCommand);
  ...
  function end(event: PointerEvent<HTMLDivElement>, cancel = false) {
    ...
    if (gesture.tool === "brush" || gesture.tool === "eraser") {
      ...
      send({ type: "action", action: { type: "shape", shape: gesture.tool, points, mode: gesture.mode } });
    }
  }
  ...
  return createPortal(<dialog ref={dialog} className="raster-editor" aria-labelledby="raster-editor-title" onCancel={(event) => { event.preventDefault(); close(); }} onKeyDown={(event) => {
    ...
    } else if (key === "delete" || key === "backspace") { event.preventDefault(); send({ type: "action", action: { type: "delete" } }); }
    ...
    <div className="raster-layout">
      ...
      <aside className="raster-settings"><section><div className="raster-tool-heading"><h3>{TOOLS.find((item) => item.code === tool)?.label}</h3><InfoHint label="当前工具操作说明">{HINTS[tool]}</InfoHint></div>
        ...
        {tool === "wand" && <><label>容差 <output>{tolerance}</output><input type="range" min="0" max="100" value={tolerance} disabled={disabled} onChange={(event) => setTolerance(Number(event.target.value))} /></label><label className="raster-check"><input type="checkbox" checked={contiguous} disabled={disabled} onChange={(event) => setContiguous(event.target.checked)} />只选相连区域</label></>}
    ...
    <footer><span role="status">{saving ? "正在保存原尺寸 PNG…" : processing ? "正在更新画布…" : showOriginal ? "正在查看原图，切回后继续绘制" : "保存为新版本 · 不扣积分 · 原图保留"}</span><button type="button" className="raster-save" disabled={disabled || !state?.changed || !state.visible} onClick={save}>{saving ? "正在保存…" : "保存编辑结果"}</button></footer>
