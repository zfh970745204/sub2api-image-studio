...
export function ToolboxPreview({ asset, options }: { asset: Asset; options: ToolboxOptions }) {
  const [source, setSource] = useState<Blob | null>(null), [background, setBackground] = useState<Blob | null>(null);
  const [url, setUrl] = useState(""), [error, setError] = useState(""), [loading, setLoading] = useState(true), [retry, setRetry] = useState(0);
  const generation = useRef(0);
  useEffect(() => {
    const controller = new AbortController(); setSource(null); setError(""); setLoading(true);
    void selectionPixels(`/api/v1/assets/${asset.id}/selection/result`, controller.signal).then(blob => { if (!controller.signal.aborted) setSource(blob); }).catch(reason => { if (!controller.signal.aborted) { setError(reason.message); setLoading(false); } });
    return () => controller.abort();
  }, [asset.id, retry]);
  useEffect(() => {
    const controller = new AbortController(); setBackground(null); setError("");
    if (options.background === "image" && options.background_asset_id) void selectionPixels(`/api/v1/assets/${options.background_asset_id}/selection/result`, controller.signal).then(blob => { if (!controller.signal.aborted) setBackground(blob); }).catch(reason => { if (!controller.signal.aborted) { setError(reason.message); setLoading(false); } });
    return () => controller.abort();
  }, [options.background, options.background_asset_id, retry]);
  useEffect(() => {
    if (!source || (options.background === "image" && !background)) { setUrl(""); return; }
    ...
    const timer = window.setTimeout(() => worker.postMessage({ id, source, background, options }), 100);
